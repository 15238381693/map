    ...
    <div class="listDataBox">
      <div v-for="(item, index) in data" :key="index" class="listBox">
        <div class="title">
          ...
          <p>
            案卷名称：<span class="name">{{ item['ajmc'] }}</span>
          </p>
        </div>
        <p class="address">许可证号：{{ item['jsgcghxkzh'] }}</p>
        <p class="year address">
          <span>项目地址：{{ item['xmdz'] }}</span> <span></span><span>年份：{{ item['year'] }}</span>
        </p>
      </div>
    </div>
...
export default {
  ...
  created() {
    this.$axios({
      method: 'POST',
      url: '/zzscjdagl/rest/zzsrestcontroller/findGhdamlBySearchcondition',
      data: {
        index: 0,
        pagesize: 10
      }
